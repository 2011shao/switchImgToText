...
function getAliBankCheck(cardNo) {
    return new Promise(function (resolve, reject) {
        axios.get("https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=" + cardNo + "&cardBinCheck=true").then(res => {
            console.log('阿里云返回', res)

            if (res.status == 200) {
                let dic = {
                }
                const bankInfo = res.data
                dic['bankName'] = getBankNameByBankCode(bankInfo.bank);
                dic['cardType'] = bankInfo.cardType;
                dic['bankCode'] = bankInfo.bank;
                dic['cardTypeName'] = getCardTypeName(bankInfo.cardType);
                dic['bankNo'] = cardNo
                resolve(dic)
            } else {
                resolve('')
